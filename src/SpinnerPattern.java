...
public class SpinnerPattern extends JoulePattern {
	...
	protected void run(double deltaMs) {
	      ...
	      float spinRPM = this.spinRPM.getValuef();
	      this.positionSpin += spinRPM*RADIANS_PER_REVOLUTION/60.0/1000.0*deltaMs;	      
	      this.positionSpin %= 360.0;
	      ...
	      for (LXPoint p : model.points) {
	    	  ...
	    	  t2.push();
	    	  t2.translate(p.x, p.y, p.z);
	    	  LXMatrix pointMatrix = t2.getMatrix();
	    	  
	    	  LXMatrix finalMatrix = matrix.multiply(pointMatrix);
	    	  float x = finalMatrix.x();
	    	  float y = finalMatrix.y();
	    	  

	    	  float pDegrees = (float) Math.toDegrees(Math.atan(x/y));
	    	  LXVector;
	    	  LXProjection;

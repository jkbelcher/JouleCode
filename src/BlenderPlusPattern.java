...
public class BlenderPlusPattern extends JoulePattern {
    ...
    public final CompoundParameter transitionTimeSecs = 
            (CompoundParameter) new CompoundParameter("Transition Time", 10, .1, 30)
            .setDescription("Sets the duration of blending transitions between patterns");
    
    public final CompoundParameter autoCycleTimeSecs = 
            (CompoundParameter) new CompoundParameter("Cycle Time", 10, .1, 60)
    ...
    protected void run(double deltaMs) {
        ...
        if (this.isChanging) {
            ...
            for (int i = 0; i < colors.length; i++) {
                int f1 = LXColor.multiply(c1[i], LXColor.subtract(LXColor.WHITE, tmask[i]));
    ...
    protected JouleBlendPattern getNextTransitionPatternRandom() {
        ...
        // Treat all patterns as active. Improve this later to cycle through

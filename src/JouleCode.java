...
import heronarts.lx.output.LXOutput.Mode;
...
//		PApplet.main(new String[] { "--present", JouleCode.class.getName(), "--display=2"});		
	}
    	 ...
    	 registerMethod("keyEvent", this);   
    	  ...
    	  try {
			...
			PApplet.println("Loaded"
							...
							,model.clusters.size() + " clusters,"
							,model.gems.size() + " gems,"
							,"and " + model.points.length + " pixels.");
		  } catch (Exception e) {
    	  ...
    	  lx = new LXStudio(this, model, true)  {
    	    ...
    	    protected void onUIReady(LXStudio lx, LXStudio.UI ui) {
    	      ...
    	      // UIWalls is from example project.  Leaving in for now because it prevents an error.
    	  ...
    	  // Some components, like GemEdges, want to compute normals after all components are loaded.
